...
exports.updateCount = exports.default = void 0;

var _discord = require("discord.js");

var _pgPromise = require("pg-promise");

var _default = async (author, args, users, db) => {
  if (args.indexOf('start') !== -1) {
    ...
    let out = 'Last ran on ' + (parseInt(result[0].month) + 1) + '/' + result[0].day + '/2018';
    return out;
...
const updateCount = async (user, newCount, db) => {
  ...
  if (result.length === 0) {
    // Create a new entry
    await db.query('INSERT into meme_count (id, count) VALUES($1, $2)', [user, newCount]);
    return 'Added new entry!';
  } else {
    await db.query('UPDATE meme_count SET count = $1 WHERE id = $2', [result[0].count + newCount, user]);
